import {Minus} from "@netist/icons"
...
>(({className, ...props}, ref) => (
    <div className="border border-ui-border-base border-transparent w-full rounded-lg overflow-hidden">
        <table
            ref={ref}
            className={clx("text-ui-fg-subtle txt-compact-small w-full border-t-transparent", className)}
            {...props}
        />
    </div>
))
...
>(({className, ...props}, ref) => (
    <tr
        ref={ref}
        className={clx(
            "bg-ui-bg-base hover:bg-ui-bg-base-hover border-ui-border-base transition-fg border-b last:border-b-transparent",
...
>(({className, ...props}, ref) => (
    <thead
        ref={ref}
        className={clx(
            "border-ui-border-base txt-compact-small-plus [&_tr:hover]:bg-ui-bg-base border-b",
            className
        )}
        {...props}
    />
...
>(({className, ...props}, ref) => (
    <th ref={ref} className={clx("h-12 pr-3 text-left border-b", className)} {...props} />
))
...
>(({className, ...props}, ref) => (
    <tbody
        ref={ref}
        className={clx("border-ui-border-base border-b-transparent", className)}
        {...props}
    />
))
