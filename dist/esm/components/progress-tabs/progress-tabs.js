...
const ProgressIndicator = ({ status, className, ...props }) => {
    ...
    return (React.createElement("span", { ...props, className: clx("text-ui-fg-muted group-data-[state=active]/trigger:text-ui-fg-interactive", className) },
        React.createElement(Icon, null)));
};
const ProgressTabsTrigger = React.forwardRef(({ className, children, status = "not-started", ...props }, ref) => (React.createElement(ProgressTabsPrimitives.Trigger, { ref: ref, className: clx("txt-compact-small-plus transition-fg text-ui-fg-muted bg-ui-bg-subtle border-r-ui-border-base flex-1 last:border-r-transparent", "inline-flex h-14 w-full flex-1 items-center gap-x-2 border-r px-4 text-left outline-none", "group/trigger overflow-hidden text-ellipsis whitespace-nowrap", "disabled:bg-ui-bg-disabled disabled:text-ui-fg-muted", "hover:bg-ui-bg-subtle-hover", "focus:bg-ui-bg-base focus:z-[1]", "data-[state=active]:text-ui-fg-base data-[state=active]:bg-ui-bg-base", className), ...props },
    React.createElement(ProgressIndicator, { status: status }),
    children)));
ProgressTabsTrigger.displayName = "ProgressTabs.Trigger";
const ProgressTabsList = React.forwardRef(({ className, ...props }, ref) => (React.createElement(ProgressTabsPrimitives.List, { ref: ref, className: clx("flex items-center justify-between", className), ...props })));
